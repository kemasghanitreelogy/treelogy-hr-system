...
const FROM = process.env.SMTP_FROM || (USER ? `Treelogy Workspace <${USER}>` : undefined);
...
export async function sendOtpEmail(to: string, code: string): Promise<void> {
  ...
  await sendEmail({
    to,
    subject: `Kode reset kata sandi Treelogy Workspace: ${code}`,
    text: `Kode verifikasi reset kata sandi Treelogy Workspace: ${code}\nBerlaku 10 menit. Abaikan jika kamu tidak meminta reset.`,
    html,
  });
}
